  ...
  workspaceView.innerHTML = `
    <div class="workspace-hero">
      ...
      <div class="workspace-badge">
        ...
        <img src="assets/applus-logo.png" alt="北京艾普拉斯检验认证 Logo" />
      </div>
    </div>
  `;
}
  ...
  workspaceView.innerHTML = `
    <section class="guidance-page">
      ...
      <div class="guidance-foot">
        <span>APPLUS+ CERTIFICATION · INTERNAL TECHNICAL KNOWLEDGE PORTAL</span>
...
function buildKnowledgeGraphNodes(stats) {
  const nodes = [
    { id: "core", label: "Applus 知识中枢", type: "core", value: 42, route: "home", x: 0, y: 0, z: 0 },
  ];
  const links = [];
  const main = [
    ...
    <section class="knowledge-graph-page">
      <div class="kg-hero">
        <div>
          ...
          <h1>北京艾普拉斯认证知识图谱</h1>
          <p>把标准、法规、规则、作业指导书、技术领域分析、培训教材和案例经验连接成可探索、可复用的认证技术网络。</p>
        </div>
      ...
      <div class="kg-stage">
        <div class="kg-canvas-shell">
          <canvas id="knowledge-graph-canvas" aria-label="北京艾普拉斯认证知识图谱"></canvas>
          <div class="kg-hint">拖拽旋转 · 点击节点进入对应知识库</div>
        </div>
        <aside class="kg-inspector">
          <span>KNOWLEDGE INDEX</span>
          <h2>认证技术网络</h2>
          <p>中心节点代表 Applus 知识中枢，外圈节点代表八项技术底盘，外围节点代表可继续展开的知识对象。</p>
...
function initKnowledgeGraph(canvas) {
  ...
  function draw() {
    ...
    gradient.addColorStop(0, "#211d1b");
    gradient.addColorStop(0.48, "#4c433e");
    gradient.addColorStop(1, "#151211");
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, width, height);

    ctx.strokeStyle = "rgba(255, 105, 0, 0.07)";
    ctx.lineWidth = 1;
    ...
    graph.links
      ...
      .forEach(([from, to]) => {
        ...
        ctx.strokeStyle = `rgba(255, 142, 64, ${alpha * 0.38})`;
    ...
    projected
      ...
      .forEach((node) => {
        ...
        const color = node.type === "core" ? "#ff6900" : node.type === "module" ? "#746660" : "#d9d1cc";
        ctx.shadowColor = node.type === "core" ? "rgba(255,105,0,0.48)" : "rgba(255,105,0,0.24)";
        ...
        if (node.type !== "leaf" || node.scale > 0.78) {
          ...
          if (node.meta && node.scale > 0.82) {
            ...
            ctx.fillStyle = "rgba(255, 238, 226, 0.72)";
...
function initKnowledgePreview(canvas) {
  ...
  function draw() {
    ...
    projected.slice(1).forEach((point) => {
      ctx.strokeStyle = `rgba(255,105,0,${0.18 * point.alpha})`;
    ...
    projected
      ...
      .forEach((point) => {
        ...
        ctx.shadowColor = point.type === "core" ? "rgba(255,105,0,0.3)" : "rgba(116,102,96,0.16)";
        ctx.shadowBlur = point.type === "core" ? 20 : 10;
        ctx.fillStyle = point.type === "core" ? "#ff6900" : point.depth > 0.82 ? "#746660" : "#d9d1cc";
        ...
        ctx.textAlign = "center";
        ctx.fillStyle = point.type === "core" ? "#ffffff" : "#2b2623";
